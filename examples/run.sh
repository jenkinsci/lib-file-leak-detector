#!/bin/bash
# Run LeakyApp with the file-leak-detector agent attached.
...
echo "Running LeakyApp with file-leak-detector agent..."
echo "Agent options: dumpatshutdown,dumpinterval=1"
echo "Will send SIGTERM after 10 seconds."
echo "---"

java \
    -javaagent:"$AGENT_JAR=dumpatshutdown,dumpinterval=1" \
    -cp "$SCRIPT_DIR" \
    LeakyApp &
